...
**The max() operator does not amplify error** — amplification ≈ 1.0, PDS
agreement 100%. This was the main structural risk and it did not materialise.
Caveat: measured under a single-shift anchor, so it is self-consistent rather
than independently confirmed (see *Anchored mode is under-determined*).
...
| `anchored` | one known U_L | absolute U_L, with the caveat below |
| `absolute` | your own E(CO₂), E(H₂), E(H₂O) | fully rigorous |
...
disagreement is **independent of any anchor choice**. Candidate explanations,
untested from the abstract alone:
...
2. the potential-determining step varies between configurations — which is the
   paper's own thesis, circumventing the scaling relation via COOH\*/CHO\*
   rotation — so U_L is not a single linear function of one adsorption energy.

If (2) holds, `absolute` mode is not merely cleaner but **necessary**, because
only two correct shifts let the PDS vary.
...
print(v.summary())
print(v.compression_note())
print(check_against_published_band(v.u_l_true)["reason"])
```
